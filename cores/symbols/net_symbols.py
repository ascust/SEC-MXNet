...
def aconv(data, name, num_filter, vertical, pad=0, kernel=1, no_bias=False, workspace=512, lr_type="mxnet"):
...
def fullyconnected(data, name, num_hidden, no_bias=False, lr_type="mxnet"):
    with _attr_scope_lr(lr_type, 'weight'):
        weight = mx.sym.Variable('{}_weight'.format(name))
    if no_bias:
...
def conv(data, name, num_filter, pad=0, kernel=1, stride=1, dilate=1, no_bias=False, workspace=512, lr_type="mxnet"):
...
def bn(data, name, eps=1.001e-5, fix_gamma=False, use_global_stats=False, lr_type="mxnet"):
    with _attr_scope_lr(lr_type, 'bias'):
...
def maxpool(data, kernel=1, pad=0, stride=1, global_pool=False, pooling_convention='valid'):
...
def avgpool(data, kernel=1, pad=0, stride=1, global_pool=False, pooling_convention='valid'):
    return mx.sym.Pooling(data, kernel=(kernel, kernel),
                        pad=(pad,pad),
                        stride=(stride, stride),
                        pool_type="avg",
